...
### Файл лексикона (XLSX или CSV)

Лексикон — это список лемм, которые считаются "известными".
Слова из DOCX, чьи леммы есть в лексиконе, не подсвечиваются.

Обязательное условие: леммы находятся в первом столбце файла.
Рекомендуемое название столбца: `Слово`.
...
Если в файле есть дополнительные столбцы — они игнорируются:
...
Если леммы находятся не в первом столбце — укажите название явно:

```python
repo = LexiconRepository(lemma_column_name="Лемма")
```

Или через CLI:

```bash
python run_highlight.py --docx "файл.docx" --lexicon "лексикон.xlsx" --column "Лемма"
```

Нормализация (нижний регистр, ё -> е) выполняется автоматически.
Пустые строки и значения NaN пропускаются.

---

### Файл стоп-слов (`data/russian_stopwords.txt`)

Одно слово на строку. Слова из этого файла не подсвечиваются
(дополняют встроенный список стоп-слов NLTK):
...
### Файл замен (`data/replacements.txt`)

Словарь замен лемм. Формат: `ключ: значение`, по одной паре на строку.
Используется для унификации вариантов написания:
...
### Файл чёрного списка (`--black`)

Одно слово на строку. Слова из этого файла подсвечиваются чёрным цветом
(в отличие от красного для обычных незнакомых слов):
